...
def request_kline_rest(symbol, period, startTime=0, endTime=0, size=1000):
    try:
        ...
        if len(content) > 0:
            print('data >>>')
            print('   ', '[' + str(content[0]) + ', ...]')
            print('data start >>>', util.getLocaleDateStrBy13(content[0][0]))
            print('data end >>>', util.getLocaleDateStrBy13(content[len(content) - 1][0]))
        return content
    except:
        util.printExcept()
        time.sleep(61)
...
def get_kline_history(period):
    ...
    try:
        ...
        for r in symbols:
            ...
            db.commit()
            print('\033[0;30;43m', startDate, period, 'start date >>>', startDate, '\033[0m')
            print('\033[0;30;43m', startDate, period, 'current date >>>', datetime.datetime.now(), '\033[0m')
            ...
            print('\033[0;30;42m', startDate, period, 'total >>> {:,}'.format(total), '\033[0m')
    ...
    print('\033[0;30;43m', startDate, period, 'start date >>>', startDate, '\033[0m')
    print('\033[0;30;43m', startDate, period, 'end date >>>', datetime.datetime.now(), '\033[0m')
    print(startDate, period, 'period done >>>', done)
    print(startDate, period, 'period doneToken >>>', doneToken)
    print(startDate, period, 'period pending >>>', pendingPeriods)
    print('\033[0;30;42m', startDate, period, 'total >>> {:,}'.format(total), ' \033[0m')
...
def get_period_interval(period):
    ...
    if period.find('m') > -1:
        ...
        num = int(num)
        interval = num * 365 * 7 * 24 * 60 * 60 * 1000
