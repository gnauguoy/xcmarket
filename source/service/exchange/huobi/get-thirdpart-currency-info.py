...
def get_thirdpart_currency_info(fullname):
    ...
    try:
        ...
        url = 'http://www.feixiaohao.com/currencies/%s/' % (fullname)
        print('url >>>', url)
        ...
        price_cny = strip_num(matchObj.group(1))
        # print('人民币价格       >>>', price_cny)
        price_scope = strip_num(matchObj.group(2))
        ...
        high = strip_num(matchObj.group(1))
        # print('24H最高价        >>>', high)
        currency['high'] = high
        ...
        low = strip_num(matchObj.group(1))
        # print('24H最低价        >>>', low)
        currency['low'] = low
        ...
        circulating_cny_value = strip_num(matchDict['ccv'])
        # print('人民币流通市值   >>>', circulating_cny_value)
        ranking = strip_num(matchDict['r'])
        # print('市值排名         >>>', ranking)
        circulating_usd_value = strip_num(matchDict['cuv'])
        # print('美元市值         >>>', circulating_usd_value)
        circulating_btc_value = strip_num(matchDict['cbv'])
        ...
        circulating_value_percent = strip_num(matchObj.group(1))
        ...
        circulating_volume = strip_num(matchObj.group(1))
        ...
        total_volume = strip_num(matchObj.group(1))
        ...
        circulating_rate = strip_num(matchObj.group(1))
        ...
        # print(matchObj.group())
        matchDict = matchObj.groupdict()
        turn_volume = strip_num(matchDict['tv'])
        # print('24H成交额        >>>', turn_volume)
        turn_volume_ranking = strip_num(matchDict['tvr'])
        ...
        turnover_rate = strip_num(matchObj.group(1))
        ...
        print('\033[0;30;41m get_thirdpart_currency_info except >>> \033[0m')
    ...
    try:
        ...
        for c in currencies:
            ...
            if type(c) == str: # 嵌套获取出现错误的币种列表
                ...
                fullname = c[2]
            ci = get_thirdpart_currency_info(fullname)
            ...
            if len(ci) == 0:
                ...
                while retry < 5:
                    ...
                    ci = get_thirdpart_currency_info(c[2])
                    if len(ci) != 0:
                        break
                    retry += 1
                    time.sleep(2)
...
#     for c in currencies:
#
#         ci = get_thirdpart_currency_info(c)
#
#         # 如果获取失败，重试 5 次。
#         if len(ci) == 0:
#             retry = 0
...
if __name__ == '__main__':
    # main()
    # get_thirdpart_currency_info('ors-group')
    # test_error_currency()
    main_timer()
